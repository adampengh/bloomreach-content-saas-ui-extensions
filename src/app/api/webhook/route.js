...
const WEBHOOKS_API_PATH = 'management/webhooks/v1/configurations'
...
export async function DELETE(request) {
  ...
  const webhookConfigurationId = searchParams.get('webhookConfigurationId')
  if (!webhookConfigurationId) {
    const response = { status: 406, statusText: 'Not Acceptable', message: 'webhookConfigurationId query parameter is required'}
    const blob = new Blob([JSON.stringify(response, null, 2)], {
      type: 'application/json',
    });
    return new Response(blob, response)
  ...
  const response = await axios(`https://${environment}.bloomreach.io/${WEBHOOKS_API_PATH}/${webhookConfigurationId}`, {
    method: 'DELETE',
    headers: {
